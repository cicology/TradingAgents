...
| TA-003 | 0 | Honor decision and risk maximum-size caps | Verified | P0 | Platform | Yes | TA-001 | `pipeline._normalize_decision` now takes `min(KELLY_CAP, declared_max_size_pct)` before sizing; missing/non-numeric/negative `max_size_pct` is treated as zero (reject), never as unlimited. 5 tests in `test_pipeline.py`; full suite 21 passed. Commit `<pending>`. |
